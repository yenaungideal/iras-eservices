...
export class AmountInputComponent implements ControlValueAccessor, OnInit {
  ...
  writeValue(value: any) {
    if (this.type === 'amount') {
      this.value = value ? getThousandSeparatedAmount(value.toString(), this.decimalLength) : '';
    } else {
  ...
  checkInput({ currentValue }: { currentValue: string }) {
    ...
    let amountPattern = `^\\s*((\\-?\\d+(,?\\d{3})*(\\.\\d{0,${this.decimalLength}})?)|((\\-?\\d*(,?\\d{3})*(\\.\\d{1,${this.decimalLength}}))))\\s*$`;
    regexPattern = new RegExp(amountPattern);
  ...
  addCommas() {
    if (this.value) {
      this.value =
        this.type === 'amount'
          ? getThousandSeparatedAmount(this.value.toString(), this.decimalLength)
