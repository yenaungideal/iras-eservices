...
describe('IrasMonthPickerComponent', () => {
    ...
    waitForAsync(() => {
      TestBed.configureTestingModule({
        ...
        .then(() => {
          ...
          inputElement = fixture.debugElement.nativeElement.shadowRoot.querySelector('.iras-monthpicker__masked-input');
        });
    })
  );
  ...
  it('#onMonthPickerKeyDown should prevent space', () => {
    const spy = spyOn(component, 'onMonthPickerKeyDown').and.callThrough();
    var e = {
      code: 'Space',
      preventDefault: () => {
        return 'test';
    ...
    component.onMonthPickerKeyDown(e);
  ...
  it('#writeValue should change value', () => {
    ...
    expect(component.maskedInputValue).toEqual('12/2020');
  });

  it('#onDatePickerClose should work', () => {
    const spyNext = spyOn(component.monthPickerClosed$, 'next').and.callThrough();
    const spyComplete = spyOn(component.monthPickerClosed$, 'complete').and.callThrough();
  ...
  it('#onMonthSelected should change the value and emit the date picker close', () => {
    const spy = spyOn(component.datePicker, 'close').and.callThrough();
    const option = new Date('2020');
    component.onMonthSelected(option);
    expect(component.maskedInputValue).toEqual('01/2020');
    expect(spy).toHaveBeenCalled();
  });

  it('#ngOnInit should change selectedDate', () => {
    const currentYear = 2020;
    const selectedMonth = '12';
    const selectedDate = new Date(`${currentYear}-${selectedMonth}`);
